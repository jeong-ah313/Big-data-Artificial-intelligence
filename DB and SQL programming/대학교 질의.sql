...
select * from 학과;
insert into 학과 value (1,'물리학과','250-8446','자4-120');
insert into 학과 value (2,'컴퓨터공학과','250-6380','공6-205');
insert into 학과 value (3,'통계학과','250-6320','경4-101');
select * from 학과;
...
(select  학과번호 from 학과 where 학과명 = '물리학과');###중첩
select 학번, 이름 from 학생 s where exists 
(select * from 학과 d where 학과명='뭉리학과' and d.학과번호=s.학과번호); ###상관중첩질의

select 이름 from 수강 e, 학생 s where 과목번호=10 and e.학번=s.학번;	###조인
select 이름 from 학생 where 학번  not in
(select 학번 from 수강 where 과목번호=10);	###중첩/ 10번 과목을 수강하지 않은 사람
select 이름 from 학생 s where  not exists
(select * from 수강 e where 과목번호=10 and e.학번 =s.학번);	###10번과목을 수강하지 않은 학생/ 상관중첩질의