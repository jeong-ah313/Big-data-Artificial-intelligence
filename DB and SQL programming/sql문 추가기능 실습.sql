...
where dno in (select dnumber from department where dname ='Reaearch');
select fname, lname from employee as e
where exists (select * from department as d where d.dname ='Research' and
d.dnumber=e.dno);

select ssn not in (select essn from works_on where pno=2);

select ssn from employee e
where not exists (select * from works_on as w where w.pno=2 and w.essn=e.ssn);
...
select fname, lname, adress from employee, department
...
where pname='Product' and pnumber=pno and essn=ssn;
...
from emplotee, dependent where ssn=essn;
select fname, lname dependent_name, relationship
...
select dname, count(*), sum(salary), avg(salaty)
